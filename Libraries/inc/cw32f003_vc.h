...
#ifndef __CW32F003_VC_H
#define __CW32F003_VC_H
 ...
 ******************************************************************************/
#define VC_INT_PRIORITY                     ((uint32_t)3)/*VCinterruptpriority(lowestbydefault)*/
...
/** @defgroup VC_DivVref
  * @{
  */
#define VC_DivVref_VDD                      ((uint32_t)0x00000000)
#define VC_DivVref_ADC                      ((uint32_t)0x00000080)

#define IS_VC_DIV_VREF(DIV_VREF)            (((DIV_VREF)==VC_DivVref_VDD)||\
                                            ((DIV_VREF) == VC_DivVref_ADC))

/** @defgroup VC_InputP
  * @{
  * VC1                 VC2
  * 输入通道0  PC02        输入通道0 PB02
  * 输入通道1  PB00        输入通道1 PA00
  * 输入通道2  PB01        输入通道2 PA01
  * 输入通道3  PB05        输入通道3 PA04
  * 输入通道4  PB04        输入通道4 PA06
  * 输入通道5  PB03        输入通道5 PA07
  * 输入通道6  PB02        输入通道6 PC00
  * 输入通道7  PA00        输入通道7 PC01
  */
...
typedef struct
{
    uint32_t     VC_DivVref;         // 6-bit VDD divider输入参考电压选择
    uint32_t     VC_DivEn;            // 使能6-bit VDD divider
    uint8_t     VC_DivValue;        // 6-bit VDD divider设置值
}VC_DivTypeDef;
