...
uint8_t FLASH_UnlockPage( uint8_t Page_Number )
{
    ...
    CW_FLASH->PAGELOCK = 0x5A5A0000 | Currentlockstatus | ( 1UL<<(Page_Number>>2));   //解锁指定页面
    return( 0x00 );
}
 ...
 *
 * @param StartAddr : 0x00000000 - 0x00004FFF
 * @param EndAddr   : 0x00000000 - 0x00004FFF
...
uint8_t FLASH_UnlockPages( uint32_t StartAddr , uint32_t EndAddr )
{
    ...
    {
        UnlockIdx = StartAddr >> 11;  // StartAddr/512/4
...
uint8_t FLASH_ErasePages( uint32_t StartAddr , uint32_t EndAddr )
{
    ...
    if( ( StartAddr > 0x4fff ) || ( EndAddr > 0x4fff ) )
    {
        return( FLASH_ERROR_ADDR );
    }
 ...
 * //待写入地址的页面已做擦除，并解除保护
 * //返回0x00代表成功，非0代表失败
 * @param WriteAddr   : 0x00000000 - 0x00004FFF
 * @param pWrBuf
...
uint8_t FLASH_LockPage( uint8_t Page_Number )
{
    ...
    Currentlockstatus = CW_FLASH->PAGELOCK;   //读取当前锁定位
    CW_FLASH->PAGELOCK = ( (0x5A5A0000 | Currentlockstatus) & ( ~(1UL<<(Page_Number>>2))) );   //锁定指定页面
    return( 0x00 );
}
