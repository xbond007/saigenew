 ...
 * @brief 设置通道1的功能
 * 
 * @param Function ATIM_CHxA_BUFF ATIM_CHxA_CIE 
 *                 ATIM_CHxB_BUFF ATIM_CHxB_CIE 
 * @param NewState ENABLE 或 DISABLE
 */
 ...
 * @brief 设置通道4的功能
 * 
 * @param Function ATIM_CH4_BUFF ATIM_CH4_CIE                 
 * @param NewState ENABLE 或 DISABLE
 */
...
void ATIM_OC1AInit(ATIM_OCInitTypeDef * ATIM_OCInitStruct)
{
    ...
    REGBITS_MODIFY(CW_ATIM->CH1CR,  ATIM_CH1CR_BUFEA_Msk |  ATIM_CH1CR_CIEA_Msk,
...
void ATIM_OC1BInit(ATIM_OCInitTypeDef * ATIM_OCInitStruct)
{
    ...
    REGBITS_MODIFY(CW_ATIM->CH1CR, ATIM_CH1CR_CISB_Msk | ATIM_CH1CR_BUFEB_Msk |  ATIM_CH1CR_CIEB_Msk,
...
void ATIM_OC2AInit(ATIM_OCInitTypeDef * ATIM_OCInitStruct)
{
    ...
    CW_ATIM->CR_f.CISA = ATIM_OCInitStruct->OCInterruptSelect;
    REGBITS_MODIFY(CW_ATIM->CH2CR,  ATIM_CH2CR_BUFEA_Msk | ATIM_CH2CR_CIEA_Msk,
...
void ATIM_OC2BInit(ATIM_OCInitTypeDef * ATIM_OCInitStruct)
{
    ...
    REGBITS_MODIFY(CW_ATIM->CH2CR, ATIM_CH2CR_CISB_Msk | ATIM_CH2CR_BUFEB_Msk | ATIM_CH1CR_CIEB_Msk,
                   (ATIM_OCInitStruct->OCInterruptSelect << 12) | \
...
void ATIM_OC3AInit(ATIM_OCInitTypeDef * ATIM_OCInitStruct)
{
    ...
    CW_ATIM->CR_f.CISA = ATIM_OCInitStruct->OCInterruptSelect;
    REGBITS_MODIFY(CW_ATIM->CH3CR,  ATIM_CH3CR_BUFEA_Msk | ATIM_CH3CR_CIEA_Msk,
...
void ATIM_OC3BInit(ATIM_OCInitTypeDef * ATIM_OCInitStruct)
{
    ...
    REGBITS_MODIFY(CW_ATIM->CH3CR, ATIM_CH3CR_CISB_Msk | ATIM_CH3CR_BUFEB_Msk | ATIM_CH1CR_CIEB_Msk,
                   (ATIM_OCInitStruct->OCInterruptSelect << 12) | \
 ...
 */
void ATIM_OC4Init(FunctionalState NewState, uint32_t ITSelect, FunctionalState ITState, FunctionalState BufferState)
{
    assert_param(IS_ATIM_OC_IT(ITSelect));

    CW_ATIM->CH4CR = (NewState ? 1UL << 5 :0 ) | (ITSelect << 3) | \
...
/**
 * @brief 设置ATIM的ETR信号的来源
 * 
 * @param ATIM_ETRSource ATIMETR_SOURCE_GPIO
 *                       ATIMETR_SOURCE_UART1_RXD
 *                       ATIMETR_SOURCE_UART2_RXD
 *                       ATIMETR_SOURCE_VC1
 *                       ATIMETR_SOURCE_VC2
 *                       ATIMETR_SOURCE_LVD
 */
void ATIM_ETRSelect(uint32_t ATIM_ETRSource)
{
    assert_param(IS_ATIMETR_SOURCE(ATIM_ETRSource));

    CW_SYSCTRL->ATIMETR = ATIM_ETRSource;
}
...
/**
 * @brief ATIM内部级联时，ITR信号源的选择
 * 
 * @param ATIM_ITRSource 
 */
void ATIM_ITRSelect(uint32_t ATIM_ITRSource)
{
    assert_param(IS_ATIMITR_SOURCE(ATIM_ITRSource));
    CW_SYSCTRL->TIMITR_f.ATIMITR = ATIM_ITRSource;
}
